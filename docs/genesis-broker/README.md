...
| GET | `/v1/issues/{n}/status` | read-only status |
| GET | `/v1/pulls/{n}` | read-only PR |
| GET | `/v1/pulls/{n}/diff` | read-only diff |
| POST | `/v1/executions/grok/draft-pr` | S-0005 limited Grok executor (Stage 1 CODE_AND_TESTS_ONLY) |

## Limited Grok Executor (S-0005)

See [grok-executor.md](./grok-executor.md) for the composite draft-PR endpoint (T-011 Stage 1).
