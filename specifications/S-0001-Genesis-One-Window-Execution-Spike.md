...
- Planner / независимый Reviewer — через отдельные OpenAI API / xAI API;
...
12. Независимый reviewer — отдельная API-модель (OpenAI API и/или xAI API).
...
15. CEO видит: ссылку на PR, краткий verdict review, кнопки:
    ...
    - **Request changes**
    - **Reject**
16. **Request changes** создаёт понятное продолжение workflow (без потери контекста).
17. Merge **не** выполняется из S-0001. Любое будущее merge-действие требует отдельного CEO Merge Authorization (Gate 4 — не реализуется в этом spike).

### 4.7 Security & operations

18. Секреты хранятся только в защищённых настройках платформы.
19. Секреты отсутствуют в GitHub, логах workflow и экспортируемой документации.
20. Установлены месячный бюджет и hard limits API; при исчерпании — остановка со статусом BLOCKED.
...
- Запись preflight (зависимости, бюджет, лимиты).
...
| API-ключи / бюджет | Hard limits; остановка при исчерпании |
...
2. Какая API-модель — primary reviewer (OpenAI vs xAI) для первого прогона.
...
|---|---|---|---|
| 1 | 2026-07-24 | Grok — Chief Architect | Создан In Review по CEO Specification Authorization |
...
При отсутствии **критической** зависимости реализация останавливается со статусом **BLOCKED** и явным перечнем причин.
...
4. Planner и независимый Reviewer — через отдельные OpenAI API / xAI API.
...
9. T-006 остаётся BLOCKED.
10. Собственная Genesis Console и LangGraph не входят в S-0001.
11. После MVP — отдельное решение о долгосрочной платформе.
12. Секреты запрещено хранить в GitHub.
